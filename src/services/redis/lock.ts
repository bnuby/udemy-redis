import { itemLockKey } from '$services/keys';
import { randomBytes } from 'crypto';
import { client } from './client';

export const withLock = async (key: string, cb: (redisClient: Client, signal: any) => any, timeoutMs = 2000) => {
	...
	// Create the lock key
	const lockKey = itemLockKey(key);
	// Set uo a while loop to implement the retry behavior
	while(retries--) {
		...
		// ELSE bref pause (tryDelayMs) and try again
		if (!acquired)
